...
class Generator extends \yii\gii\Generator
{
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            ...
            if (preg_match('/^(updated_at|created_at|created_by|updated_by)$/i', $attribute)) {
                return "";
            }
        ...
        if (preg_match('/^(active|is_|)$/i', $attribute) || $column->phpType === 'boolean') {
            return "\$form->field(\$model, '$attribute')->widget(SwitchInput::class, [
    ...
    {
        if ($column->phpType === 'boolean' || $column->name === 'active' || stripos($column->name, 'is_')) {
            return 'boolean';
        }

        if ($column->phpType === 'string' && in_array($column->name, ['image', 'logo', 'photo', 'avatar']) || stripos($column->name, 'image') != false) {
            return 'image';
        }
        ...
        if ((stripos($column->name, 'time') !== false && $column->phpType === 'integer') || stripos($column->name, '_at') !== false) {
            return 'datetime';
        }
    ...
    public function generateSearchRules()
    {
        ...
        foreach ($table->columns as $column) {
            switch ($column->type) {
                ...
                default:
                    if (!in_array($column->name, ['image', 'logo', 'photo', 'avatar'])) {
        ...
        foreach ($columns as $column => $type) {
            switch ($type) {
                ...
                case Schema::TYPE_DATETIME:
                    ...
                    break;
                default:
                    if ($type !== Schema::TYPE_TIMESTAMP && !in_array($column, ['image', 'logo', 'photo', 'avatar'])) {
     ...
     * @return bool
     */
    public function hasBooleanColumn()
    {
        if ($table = $this->getTableSchema()) {
            $columnTypes = ArrayHelper::getColumn($table->columns, 'type', false);
            if (in_array('active', $table->columnNames) || in_array(Schema::TYPE_BOOLEAN, $columnTypes)) {
                return true;
            }
        }
        return false;
